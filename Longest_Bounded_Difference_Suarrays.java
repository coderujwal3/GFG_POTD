class Solution {
  public ArrayList<Integer> longestSubarray(int[] arr, int x) {
        ...
        for(int end = 0; end<arr.length;end++) {
            while(!maxDq.isEmpty() && arr[maxDq.peekLast()] <= arr[end]) maxDq.pollLast();
            maxDq.addLast(end);
            while(!minDq.isEmpty() && arr[minDq.peekLast()] >= arr[end]) minDq.pollLast();
            minDq.addLast(end);
        ...
        for(int i=ansStr;i<ansStr+maxLen; i++) ans.add(arr[i]);
        return ans;
    }
}
